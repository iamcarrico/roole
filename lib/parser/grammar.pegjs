...
pseudoSelector
	= ':' doubled:':'? name:identifier argument:pseudoArguments? {
		return new N('pseudoSelector', [name, argument || null], {doubled: !!doubled});
	}

pseudoArguments
	= '(' _ argument:pseudoArgument _ ')' {
		return argument;
	}
...
identifier
	= values:(rawIdentifier / d:'-'? v:(member / variable / interpolation) {return d ? [d,v] : v;})+ {
...
member
  = variable:variable '[' range:range ']' {
    return new N('member', [variable, range]);
  }
...
rawNumber = neg:"-"? value:$([0-9]* '.' [0-9]+ / [0-9]+) {
		return (neg ? -1 : 1) * value
	}
...
restParameter
	= '...' variable:variable {
		return new N('restParameter', [variable]);
	}
...
assignment
	=  name:member _ operator:$([-+*/?]? '=') _ value:list _ semicolon {
...
keyframeList
	= '{' _ keyframeRules:keyframeRules? _ '}' {
		return new N('keyframeList', keyframeRules || []);
	}
...
propertyList
	= '{' propertyRules:(_ p:propertyRules {return p})? _ '}' {
		return new N('propertyList', propertyRules || []);
	}
